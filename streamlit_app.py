...
class EmploymentDataProcessor:
    ...
    def load_data(_self) -> bool:
        """CSV 파일을 읽고 기본 전처리 수행 (캐시 적용)"""
        try:
            show_progress("📂 파일 읽는 중...", 10)
            
            # 파일 존재 확인
            if not _self._check_file_exists():
                return False
            
            # 데이터 읽기
            df = _self._read_csv_with_encoding()
            if df is None:
                return False
            ...
            # 데이터 검증 및 품질 리포트 생성
            _self.raw_df = df.copy()
            _self.data_quality_report = _self._generate_quality_report(df)
            
            if not _self._validate_data(df):
                return False
            
            show_progress("🧹 데이터 정제 중...", 50)
            
            # 데이터 필터링 및 정제
            df_filtered = _self._filter_data(df)
            df_cleaned = _self._clean_data(df_filtered)
            
            show_progress("📊 최종 처리 중...", 80)
            
            # 최종 데이터 저장
            _self.df = df_cleaned
            _self.employed_df = df_cleaned[df_cleaned['취업구분1'] == '취업'].copy()
    ...
    def _read_csv_with_encoding(self) -> Optional[pd.DataFrame]:
        ...
        for encoding in DATA_CONFIG['encoding_options']:
            try:
                ...
                st.info(f"📄 파일 인코딩: {encoding.upper()}")
                return df
            except UnicodeDecodeError:
                continue
        
        show_message("❌ 지원되는 인코딩으로 파일을 읽을 수 없습니다.", "error")
        return None
    ...
    def _generate_quality_report(self, df: pd.DataFrame) -> Dict:
        ...
        report = {
            ...
            'missing_values': missing_values[missing_values > 0],  # 결측값이 있는 컬럼만
            'duplicate_records': df.duplicated().sum(),
            'data_types': df.dtypes.to_dict(),
            'unique_values': {col: df[col].nunique() for col in df.columns},
            'year_range': (df['조사년도'].min(), df['조사년도'].max()) if '조사년도' in df.columns else None
        }
        return report
